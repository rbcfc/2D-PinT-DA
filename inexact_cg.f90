...
!
! Description:
!   Implements the inexact Conjugate Gradient (CG) method using Parareal
!   matrix-vector products as approximations. Also provides the
!   standard exact CG routine, along with helper routines for computing
!   residuals, cost functions, norms, and stopping tolerances.
!
! Dependencies:
!   - parareal_utils
!   - eigenvalues
...
subroutine inexact_conjgrad(b,x,n,eps,nitermax,observation)
  ...
  integer :: iteration, d, k, i, j, it_out, ksol
  real :: alpha, beta_old, beta_new, phi, phi_new, big_phi_new
  real :: omega, omega_hat, omega2, phi_hat
  real :: bnorm, pnorm, r2norm, rinvnorm, bound, exact_pnorm, exact_bnorm, exact_ebound
  real :: xout, xout1, xout2, q, cf, q2, cf_temp
  real :: e, e1, c2, c4, c5, c6, res2

  real :: big_phi = 1.

  real ,dimension(n) :: x_new, ep_k, ep_k1, c, c1
  real, dimension(n) :: r, p, p_new, r_new
  real, dimension(n) :: temp, tmp, tmp1, gap1, gap2 
  real, dimension(n) :: ep_temp, ep_temp1

  real, dimension(:), allocatable :: q1, sol1
  real, dimension(:,:), allocatable :: cf1, para_sol, u
  ...
  do while (.true.)
    ...
    !~~ Matrix-vector product: exact on iteration 0, Parareal for subsequent iterations ~~
    ...
    if (iteration .gt. 0) then
      ...
      if (allocated(para_sol)) deallocate(para_sol)
      ...
      print *, 'Parareal iterations: ', it_out
      write(*,*)
      
      do k = 1,N_time_windows

        ! Loop over Parareal iterates to find the earliest iteration k that satisfies
        ! ||P(k)p_j - P(k+1)p_j||_2 <= \xi_j

        c2 = 0.0
        do i = 1, Nobs
          c1 = all_iter(:,obs_windows(i),k)
        ...
        write(*,*)
        if (regularisation) then
          call weighting_matrix(n,p,tmp)
          pnorm = sqrt(c2 + alpharegul*DOT_PRODUCT(p,tmp))
        else
          pnorm = sqrt(c2)
        end if
        print *, 'approx pnorm at Parareal iteration ', k, '=', pnorm

        call E_bound(eps,exact_bnorm,pnorm,r2norm,phi,xout)
        xout1 = xout*pnorm 
        print *, 'omega2 with approx pnorm ',xout1

        call E_bound(eps,bnorm,exact_pnorm,r2norm,phi,xout)
        xout2 = xout*exact_pnorm
        print *, 'omega2 with approx bnorm ',xout2

        call E_bound(eps,bnorm,pnorm,r2norm,phi,xout)
        omega = xout
        omega2 = omega*pnorm
        print *, 'omega = ', omega, 'omega2 = ', omega2
        ...
        do i = 1, Nobs
          ep_temp = all_iter(:,obs_windows(i),k)
          ep_temp1 = all_iter(:,obs_windows(i),k+1)
        ...
        print *, 'approx ep norm at Parareal iteration ', k, ' = ', e
        ...
        if (e < omega2) then
          ...
          print *, 'Parareal iterations accepted at k+1 = ', k+1
          
          c4 = 0.0
          do i = 1, Nobs
            temp = all_iter(:,obs_windows(i),k+1)
          ...
          if (regularisation) then
            call weighting_matrix(n,p,tmp)
            pnorm = sqrt(c4 + alpharegul*DOT_PRODUCT(p,tmp))
          else
            pnorm = sqrt(c4)
          end if
          print *, 'accepted pnorm at Parareal iteration ', k+1, '=', pnorm
          ksol = k
          exit
        end if
      end do
      ...
      do k =1,ksol
        e1 = dnrm2(n,para_sol(:,k) - tmp1,1)
        print *, 'exact ep norm at parareal iteration ', k, ' = ', e1
      ...
      c = 0.
      do i = 1, Nobs
        tmp = all_iter(:,obs_windows(i),ksol)
        where (.not. obs_mask(:, i)) tmp = 0.0

        do j = obs_windows(i)-1,0,-1
          call INTEGRATE_SW_ADJOINT(xnb=tmp,nt=curgrid%nt_time_windows)
        end do
        c = c + tmp
      end do
    ...
    if (iteration .gt. 0) then
      ...
      if(present(observation)) then
        ...
        cf = 0.0
        do i = 1,Nobs
          cf_temp = 0.5*(dnrm2(n,cf1(:,i) - observation(:,i),1))**2
          cf = cf + cf_temp
        end do
        if (regularisation) then
    ...
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ! Stopping criterion (approximate quadratic decrease)
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ! Re-orthogonalisation and search direction update
    !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
